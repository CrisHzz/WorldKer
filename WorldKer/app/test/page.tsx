...
import { CalendarDays, LogOut, Rocket, Settings, User, ThumbsUp, MessageSquare } from "lucide-react";
...
export default function Dashboard() {
    const [posts, setPosts] = useState([
        ...
        { 
            ...
            rockets: 7,
            comments: [
                { name: "Jhonathan G", comment: "Alguien que le cancele la materia porfavor" },
        ...
        { 
            ...
            rockets: 4,
            comments: [
                { name: "Phol", comment: "Apoyo el aporte" }
            ]
        }
    ]);
            ...
            <nav className="bg-primary/20 border-b border-primary/20 backdrop-blur-sm absolute top-0 w-full h-16 px-4 flex items-center z-50">
                <div className="container mx-auto flex items-center justify-between">
                    ...
                    <ul className="flex items-center space-x-4">
                        ...
                        <li>
                            <Image src="/wvelez.jpg" alt="Profile" width={40} height={40} className="rounded-full border-2 border-primary cursor-pointer hover:scale-110 transition-transform duration-200" />
                        </li>
                    </ul>
                </div>
            </nav>
                    ...
                    <aside className="w-48 mr-4 hidden md:block">
                        <div className="bg-transparent border border-white rounded-lg p-4">
                            <ul className="space-y-2">
                                <li><Link href="#" className="flex items-center space-x-2 p-2 rounded hover:bg-primary/30 transition"><Rocket size={18} /><span>Dashboard</span></Link></li>
                                <li><Link href="#" className="flex items-center space-x-2 p-2 rounded hover:bg-primary/30 transition"><Settings size={18} /><span>Settings</span></Link></li>
                                <li><Link href="#" className="flex items-center space-x-2 p-2 rounded hover:bg-primary/30 transition"><LogOut size={18} /><span>Logout</span></Link></li>
                            </ul>
                        </div>
                    </aside>
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex items-center space-x-4">
                                        <Image src={rocket.img} alt={`Rocket ${i + 1}`} width={50} height={50} className="rounded-full" />
                                        <div>
                                            <div className="font-semibold">{rocket.name}</div>
                                            <div className="text-sm text-primary-foreground/70">Number: {rocket.number}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                {posts.map((post, i) => (
                                    <div key={i} className="bg-primary/20 rounded p-4">
                                        <div className="flex items-center space-x-4 mb-2">
                                            <Image src={post.img} alt={post.name} width={40} height={40} className="rounded-full" />
                                            <div className="font-semibold">{post.name}</div>
                                        </div>
                                        <p className="mb-4">{post.comment}</p>
                                        <div className="flex space-x-4 mb-4">
                                            <button onClick={() => handleLike(post.id)} className="flex items-center space-x-1 text-sm">
                                                <ThumbsUp size={16} />
                                                <span>{post.likes}</span>
                                            </button>
                                            <button onClick={() => handleRocket(post.id)} className="flex items-center space-x-1 text-sm">
                                                <Rocket size={16} />
                        ...
                        <div className="bg-transparent border border-white rounded-lg p-4">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex items-center space-x-4">
                                        <Image src={member.img} alt={`Team Member ${i + 1}`} width={50} height={50} className="rounded-full" />
